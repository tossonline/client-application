...
{
    internal sealed class AnalyticsArchiveJob : BackgroundJob
    {
        private readonly IObservabilityManager<AnalyticsArchiveJob> _observabilityManager;
        private readonly IAnalyticsConfiguration _serviceConfiguration;
        private readonly IStoredProcedureExecutor _storedProcedureExecutor;

        public AnalyticsArchiveJob(IAnalyticsConfiguration serviceConfiguration,
            IStoredProcedureExecutor storedProcedureExecutor,
            IObservabilityManager<AnalyticsArchiveJob> observabilityManager)
        {
        ...
        public override async Task ExecuteWork()
        {
            ...
            using (ActivityBuilder builder = _observabilityManager.CreateActivity(TraceNames.ARCHIVE).Start())
            {
                try
                {
                    ArchivingJobConfiguration? configuration = _serviceConfiguration.BackgroundServices.JobConfigurations.ArchivingJobConfiguration?[nameof(AnalyticsArchiveJob)];
                    ...
                    await _storedProcedureExecutor.Run(new StoredProcedureOptions
                    {
                        ...
                        Procedure = configuration.JobStoredProcedures["ArchiveStoredProcedure"],
                        Parameters = new
                        {
                    ...
                    _observabilityManager
                        .Counter
                        .Increment(MetricMeasureNames.CounterNumberOfArchiveExecutions.Name);

                    builder.AddEvent(EventNames.ARCHIVE_COMPLETED);
                    _observabilityManager.LogMessage(InfoMessages.ARCHIVE_COMPLETED).AsInfo();
                }
                catch (Exception ex)
                {
                    _observabilityManager.LogMessage(ErrorMessages.BACKGROUND_JOB_ARCHIVE_ERROR).AsError(ex);
                    _observabilityManager
                        .Counter
                        .Increment(MetricMeasureNames.CounterNumberOfBackgroundJobExceptions.Name);

                    throw new BackgroundJobException(ErrorMessages.BACKGROUND_JOB_ARCHIVE_ERROR, ex);
                }
            }
        }
    }
