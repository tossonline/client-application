...
-- PixelEvents Indexes (high-volume table - most critical)
...
CREATE INDEX [IX_PixelEvents_EventType_CreatedAt] ON [dbo].[PixelEvents] ([EventType], [CreatedAt]);

-- Players Indexes (for PlayerAggregate queries)
...
-- EventSummaries Indexes (for MetricsAggregate and CampaignAggregate)
...
-- CampaignMetrics Indexes (for CampaignAggregate performance queries)
...
-- AnomalyDetections Indexes (for AnalyticsAggregate anomaly queries)
...
-- GeoAnalytics Indexes (for AnalyticsAggregate geographic queries)
...
PRINT 'All indexes created successfully for analytics performance optimization';
PRINT 'Optimized for: CampaignAggregate, PlayerAggregate, MetricsAggregate, and AnalyticsAggregate queries';